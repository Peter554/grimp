...
fn _rustgrimp(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    ...
    m.add_class::<GraphModuleNames>()?;
    m.add_class::<GraphModuleNamesIterator>()?;
...
impl GraphWrapper {
    ...
    pub fn add_module(&mut self, module: &str, is_squashed: bool) -> PyResult<()> {
        ...
        if self.get_visible_module_by_name(module).is_ok()
            && self.is_module_squashed(module)? != is_squashed
        {
